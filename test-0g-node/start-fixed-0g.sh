...
echo "🚀 Starting FIXED 0G Chain Node (Both Layers Properly Configured)"

# Set environment variables
export DATA_DIR="/data"
export NODE_NAME="${NODE_NAME:-render-0g-fixed}"
...
while true; do
    ...
    if [ $((SECONDS % 60)) -eq 0 ]; then
        CONSENSUS_BLOCK=$(curl -s http://localhost:26657/status | jq -r '.result.sync_info.latest_block_height' 2>/dev/null || echo "unknown")
        EXECUTION_BLOCK=$(curl -s -X POST http://localhost:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' | jq -r '.result' 2>/dev/null || echo "unknown")
        echo "📈 Consensus: Block $CONSENSUS_BLOCK | Execution: Block $EXECUTION_BLOCK"
    fi
