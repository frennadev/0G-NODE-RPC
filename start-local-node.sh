...
echo "🚀 Starting Local 0G Node (macOS Compatible)"

# Set environment variables for local testing
export DATA_DIR="./local-data"
export NODE_NAME="local-0g-node"
...
if [ ! -f "$DATA_DIR/0g-home/0gchaind-home/config/node_key.json" ]; then
    ./aristotle-v1.0.0/bin/0gchaind init $NODE_NAME --home $DATA_DIR/tmp
...
if [ ! -f "$DATA_DIR/0g-home/geth-home/geth/chaindata/CURRENT" ]; then
    ./aristotle-v1.0.0/bin/geth init --datadir $DATA_DIR/0g-home/geth-home ./aristotle-v1.0.0/geth-genesis.json
...
if [ ! -f "$DATA_DIR/0g-home/0gchaind-home/config/jwt.hex" ]; then
    ./aristotle-v1.0.0/bin/0gchaind jwt generate --home $DATA_DIR/0g-home/0gchaind-home
...
if [ -f "$DATA_DIR/0g-home/0gchaind-home/config/config.toml" ]; then
    sed -i.bak "s/moniker = \"local-0g-node\"/moniker = \"$NODE_NAME\"/" $DATA_DIR/0g-home/0gchaind-home/config/config.toml
fi
...
./aristotle-v1.0.0/bin/0gchaind start \
    --rpc.laddr tcp://0.0.0.0:26657 \
    --chaincfg.kzg.trusted-setup-path=aristotle-v1.0.0/kzg-trusted-setup.json \
...
echo "🎉 Local 0G Chain Node started successfully!"
echo "📊 RPC Endpoints:"
